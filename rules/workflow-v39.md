...
- **决策闸门**: 涉及业务口径/验收标准/模拟实盘一致性的变更，必须先获老板明确决策，方可发布 Trae 任务。
...
- **RCA DoD (三件套)**: 必须包含 1.复现步骤 2.冒烟枪证据(Smoking Gun) 3.排除法表(≥2备选)。缺一不可。
- **原子写入 (Atomic I/O)**: 关键文件 (Config/State/Evidence) 必须用 **Write Temp -> Rename**。Verify 必须检查 JSON 可解析性。
- **显式终态 (Explicit Termination)**: 循环/重试逻辑必须定义不可逆 `TERMINAL` 状态。Server terminated -> Client 必须进入 TERMINAL 并停止自动重连。Verify 必须断言状态闭环。

6. 智能体交付协议 (Agent Delivery Protocol)
- **主动回报 (Proactive Reporting)**: 任务结束时，必须主动生成并展示 `report_for_chatgpt.txt` (Full Envelope 格式)，包含 RESULT_JSON, LOG_HEAD, LOG_TAIL, INDEX。
...
- **文件清单**: 必须包含 `healthcheck_53121.txt` (含 `/` 与 `/pairs` 200 OK) + 至少一项业务证据 (`ui_copy_details*.json` / `sse_capture*.out` / `manual_verification.json`)。
