...
- **说明**：这是里程碑分支标签，用于提醒上下文。
- **Git 分支对应规则 (One-to-One)**:
  - 每个里程碑分支标签 **必须对应且仅对应** 一个 GitHub 远端分支。
  - **默认禁止**“每个任务都新建 GitHub 分支”。同一标签内的多个任务必须在 **同一个 GitHub 分支** 与 **同一个 PR** 上迭代。
  - **仅当**进入新的里程碑分支标签时，才允许新建 GitHub 分支。
  - **例外**：如确需在当前标签下新建分支，必须在任务目标明确说明“为什么不能复用当前分支”。
